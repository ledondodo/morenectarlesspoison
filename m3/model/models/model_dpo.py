...
class AutoDPOModelForCausalLM(PreTrainedModelWrapper):
    ...
    def get_logprobs(self, batch, tokenizer):
        ...
        input_ids = tokenizer(batch["prompt"], return_tensors="pt", max_length=512, truncation=True, padding="max_length").input_ids
        chosen_tokenized = tokenizer(batch["chosen"], return_tensors="pt", max_length=512, truncation=True, padding="max_length").input_ids
        rejected_tokenized = tokenizer(batch["rejected"], return_tensors="pt", max_length=512, truncation=True, padding="max_length").input_ids

        # get the logits
        with torch.no_grad():
            logits = self.pretrained_model(input_ids=input_ids).logits
        
        chosen_tokenized = chosen_tokenized[:, 1:].clone()
        rejected_tokenized = rejected_tokenized[:, 1:].clone()

        logits = logits[:, :-1, :]

        loss_mask_chosen = chosen_tokenized != tokenizer.pad_token_id
        loss_mask_rejected = rejected_tokenized != tokenizer.pad_token_id

        chosen_tokenized[chosen_tokenized == tokenizer.pad_token_id] = 0
        rejected_tokenized[rejected_tokenized == tokenizer.pad_token_id] = 0

        per_token_chosen_logps = torch.gather(logits.log_softmax(-1), dim=2, index=chosen_tokenized.unsqueeze(2)).squeeze(2)
        per_token_rejected_logps = torch.gather(logits.log_softmax(-1), dim=2, index=rejected_tokenized.unsqueeze(2)).squeeze(2)

        chosen_logps = (per_token_chosen_logps * loss_mask_chosen).sum(-1) / loss_mask_chosen.sum(-1)
        rejected_logps = (per_token_rejected_logps * loss_mask_rejected).sum(-1) / loss_mask_rejected.sum(-1)
        ###############################################################
    ...
    def prediction_step_mcqa(self, batch, tokenizer):
        ...
        try:
            ...
            raise ImportError(
                "The `outlines` library needs to be installed in order to use `mcqa` prediction."
            )
    
        model_mcqa = Transformers(self.pretrained_model, tokenizer)
        choices = ['A','B','C','D']
        generator = outlines.generate.choice(model_mcqa, choices)

        for q in batch['question']:
            out = generator(q)
            output_dict['preds'].append(out)
